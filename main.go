...
func main() {
	...
	withLogin.GET("/show", getContextInfoHandler)
	withLogin.GET("/countriesss/:countryCode", getCountryCitiesInfoHandler)
	withLogin.POST("/write", postWriteHandler)
...
type WriteBody struct {
	...
	Contexts string `json:"contexts,omitempty"  form:"contexts" db:"Contexts"`
}

type WriteBodyForShow struct {
	Username sql.NullString `json:"username,omitempty"  form:"username" db:"Username"`
	Contexts sql.NullString `json:"contexts,omitempty"  form:"contexts" db:"Contexts"`
}
...
func postWriteHandler(c echo.Context) error {
	req := WriteBody{}
	sess, _ := session.Get("sessions", c)

	req.Username = sess.Values["userName"].(string)
	c.Bind(&req)

	_, err := db.Exec("INSERT INTO contexts (Contexts) VALUES (?)", req.Contexts)
	if err != nil {
...
func getContextInfoHandler(c echo.Context) error {
	writeBody := WriteBodyForShow{}
	var Contexts []WriteBodyForShow
	rows, _ := db.Queryx("SELECT * FROM contexts")
	for rows.Next() {
		err := rows.StructScan(&writeBody)
		if err != nil {
			return c.String(http.StatusInternalServerError, fmt.Sprintf("db error: %v", err))
		}
		contexts := WriteBodyForShow{writeBody.Username, writeBody.Contexts}
